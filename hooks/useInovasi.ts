...
export function useInovasiList() {
  ...
  useEffect(() => {
    ...
    const url = joinUrl(BASE, "inovasi");
...
export function useInovasiItem(id?: number | string | null) {
  const [data, setData] = useState<BeritaItemUI | null>(null);
  ...
  useEffect(() => {
    ...
    let mounted = true;
    const BASE = process.env.NEXT_PUBLIC_API_URL;
    const urlById = joinUrl(BASE, `inovasi/${id}`);
    ...
    fetch(urlById)
      .then(async (res) => {
        ...
        if (res.status === 404) {
          const fallback = joinUrl(BASE, "inovasi");
