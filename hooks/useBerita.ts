...
function mapApiToUi(item: any): BeritaItemUI {
  const firstImage =
    Array.isArray(item.berita_gambar) && item.berita_gambar.length > 0
      ? item.berita_gambar[0].url_gambar
      : undefined;
...
export function useBeritaItem(id?: number | string | null) {
  const [data, setData] = useState<BeritaItemUI | null>(null);
  ...
  useEffect(() => {
    ...
    let mounted = true;
    const BASE = process.env.NEXT_PUBLIC_API_URL;
    const urlById = joinUrl(BASE, `berita/${id}`);
    ...
    fetch(urlById)
      .then(async (res) => {
        ...
        if (res.status === 404) {
          const fallback = joinUrl(BASE, "berita");
