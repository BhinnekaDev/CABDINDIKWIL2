...
function mapApiToUi(item: any): BeritaItemUI {
  const firstImage =
    Array.isArray(item.cerita_praktik_baik_gambar) && item.cerita_praktik_baik_gambar.length > 0
      ? item.cerita_praktik_baik_gambar[0].url_gambar
      : undefined;
...
export function useCeritaPraktikBaikList() {
  ...
  useEffect(() => {
    ...
    const url = joinUrl(BASE, "cerita-praktik-baik");
...
export function useCeritaPraktikBaikItem(id?: number | string | null) {
  const [data, setData] = useState<BeritaItemUI | null>(null);
  ...
  useEffect(() => {
    ...
    let mounted = true;
    const BASE = process.env.NEXT_PUBLIC_API_URL;
    const urlById = joinUrl(BASE, `cerita-praktik-baik/${id}`);
    ...
    fetch(urlById)
      .then(async (res) => {
        ...
        if (res.status === 404) {
          const fallback = joinUrl(BASE, "cerita-praktik-baik");
