...
import { useSeputarCabdinItem } from "@/hooks/useSeputarCabdin";

export default function SeputarCabdinDetailPage() {
  const { id } = useParams();
  const { data: berita, loading, error } = useSeputarCabdinItem(id as string);
    ...
    return (
      <>
        ...
        <div className="p-6">
          ...
          <Link href="/SeputarCabdin" className="text-blue-600 underline">
            ← Kembali ke Seputar CABDIN
          </Link>
        </div>
        <Footer />
