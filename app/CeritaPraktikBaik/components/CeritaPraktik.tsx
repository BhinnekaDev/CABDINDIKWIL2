...
import { useCeritaPraktikBaikList } from "@/hooks/useCeritaPraktikBaik";

export default function CeritaPraktikBaik() {
  const { data: berita, loading } = useCeritaPraktikBaikList();
  return (
    <div className="relative py-6">
      ...
      <div className="relative z-10">
        <div className="text-center mb-6">
          <h1 className="text-3xl font-extrabold mb-4">Cerita Praktik Baik</h1>
          <p className="max-w-2xl mx-auto text-gray-700">
            Jelajahi berbagai cerita praktik baik dalam dunia pendidikan yang
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-4">
          ...
          {!loading &&
            (berita ?? []).map((item) => (
              <Link
                key={item.id}
                href={`/CeritaPraktikBaik/${item.id}`}
